...
export default function ChapterForm({ courses, initial, mode }: ChapterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(
      mode === "create" ? "/api/admin/chapters" : `/api/admin/chapters/${initial?.id}`,
      {
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-6">
      ...
      <div>
        ...
        <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} dir="rtl" className="w-full rounded-lg border border-gray-300 px-3 py-2 font-urdu" />
      </div>
