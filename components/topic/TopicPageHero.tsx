...
export default function TopicPageHero({
  title,
  topicOrder,
  chapterTitle,
  chapterSlug,
  ...
  const startHref = firstSubtopicSlug
    ? `/subtopic/${firstSubtopicSlug}`
    : null;

  const stats = [
    { icon: FileText, value: toUrduNumeral(subtopicCount), label: "سبق" },
    { icon: Clock, value: readingTime, label: "تخمینی وقت" },
    { icon: Brain, value: "تفصیلی", label: "سطح" },
  ];

  const highlights = [
    { icon: BookOpen, text: "اردو وضاحت" },
  ...
  return (
    <section
      ...
      aria-labelledby="topic-hero-title"
    >
      <div
      ...
      <div className="container-public relative z-10 py-10 lg:py-12">
        ...
        >
          ...
          <Link
            ...
            className="line-clamp-1 max-w-[8rem] transition-colors hover:text-blue-600 dark:hover:text-blue-400 sm:max-w-[10rem]"
          >
            {courseTitle}
          </Link>
          <ChevronLeft className="h-4 w-4 rotate-180 opacity-60" aria-hidden />
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0 flex-1 text-center lg:text-right">
            <div className="flex flex-wrap items-center justify-center gap-2 lg:justify-start">
              <Link
                href={`/chapter/${chapterSlug}`}
                className="inline-flex items-center gap-1.5 rounded-full border border-blue-200/80 bg-blue-50 px-3 py-1.5 font-sans text-caption font-medium text-blue-800 transition-colors hover:bg-blue-100 dark:border-blue-800 dark:bg-blue-950/80 dark:text-blue-200 dark:hover:bg-blue-900/60"
              >
                <Layers className="h-3.5 w-3.5 shrink-0" aria-hidden />
                <span className="line-clamp-1 max-w-[12rem]">{chapterTitle}</span>
              </Link>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-violet-200/80 bg-violet-50 px-3 py-1.5 font-sans text-caption font-semibold text-violet-800 dark:border-violet-800 dark:bg-violet-950/50 dark:text-violet-300">
                موضوع {toUrduNumeral(topicOrder)}
              </span>
            ...
            <h1 id="topic-hero-title" className="section-title-urdu mt-5">
              {title}
            </h1>

            <p className="body-urdu-comfort mx-auto mt-4 max-w-2xl text-base lg:me-0 lg:ms-0">
              اس موضوع میں اردو وضاحت، ریاضی، حقیقی مثالیں، اور عملی Python کوڈ
              شامل ہے — نیچے سے سبق ترتیب وار پڑھیں۔
            </p>
